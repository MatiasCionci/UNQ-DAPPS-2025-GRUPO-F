...
public class ScraperServicePlayers {
    ...
    public List<PlayerBarcelona> scrapeAndSavePlayers() {
        ...
        options.addArguments(
            "--headless=new",
            "--no-sandbox",
            "--disable-dev-shm-usage", // Sigue siendo importante, junto con shm_size externo
            "--disable-gpu",
            "--disable-extensions",
            "--window-size=1920,1080"
            // --user-agent y --user-data-dir comentados para simplificar
        );
        ...
        try {
            ...
            // Aumentar ligeramente el timeout general por si el entorno es muy lento
            ...
            WebElement selectElem = null; // Declarar fuera del try/catch interno
            try {
                By torneoLocator = By.cssSelector("select[data-backbone-model-attribute-dd='tournamentOptions']");

                // Pausa breve por si hay overlays desapareciendo
                ...
                try {
                    ...
                    // Pequeña pausa después del scroll para que el navegador se estabilice
                    try { Thread.sleep(300); } catch (InterruptedException ignored) {}
                    log.debug("Scroll hacia el selector completado.");
                } catch (Exception scrollEx) {
                    // No es crítico si el scroll falla, pero puede ser útil saberlo
                    log.warn("No se pudo forzar el scroll hacia el selector: {}", scrollEx.getMessage());
                }

                // --- Intento con Select y Fallback JS (como en v7) ---
                ...
                try {
                    ...
                    // Re-localizar el elemento antes de usar JS si fue Stale
                    if (enie instanceof StaleElementReferenceException) {
                         log.debug("Re-localizando elemento stale antes del fallback JS...");
                         try {
                             // Usa una espera corta para re-encontrar, por si tarda en reaparecer
                    ...
                    // Asegurarse de que el elemento re-encontrado (o el original si no era stale) no sea null
                    if (selectElem == null) {
                         throw new RuntimeException("Fallo crítico: selectElem es null antes del fallback JS.");
                    }
                    try {
                        // Fallback: Usar JavaScript para cambiar el valor y disparar el evento 'change'
                        // El valor '8' corresponde a LaLiga en WhoScored (verificar inspeccionando el HTML si falla)
                        ...
                        // Pausa más larga después de JS porque la actualización puede ser más lenta
                ...
                // --- Espera por la actualización de la tabla (sin cambios) ---
                ...
                log.debug("Pausa post-selección completada.");
                 ...
                 // Timeout esperando la espera personalizada (lambda)
                 ...
                 takeScreenshot(driver, baseScreenshotPath + "_select_custom_wait_timeout.png"); // Nombre de screenshot específico
                 throw new RuntimeException("Timeout crítico esperando la disponibilidad del selector de torneo.", toe);
            } catch (Exception e) {
                // Captura otras excepciones durante la selección
            ...
            // Extracción (Sin cambios respecto a v7)
            ...
            // ... (resto del bucle for para procesar filas igual que antes) ...
